...
def non_max_suppression(
    ...
    soft_nms: bool = False,  # 新增：是否使用 Soft NMS
    soft_nms_method: str = 'gaussian',  # Soft NMS 方法：'linear' 或 'gaussian'
    soft_nms_sigma: float = 0.5,  # Gaussian 方法的 sigma
):
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if rotated:
            ...
            if soft_nms:
                # 使用 Soft NMS
                i = TorchNMS.soft_nms(boxes, scores, iou_thres, sigma=soft_nms_sigma, method=soft_nms_method)
...
class TorchNMS:
    ...
    def soft_nms(
        ...
        """Soft NMS implementation that decays scores instead of suppressing boxes.

        Args:
            boxes (torch.Tensor): Bounding boxes with shape (N, 4) in xyxy format.
            scores (torch.Tensor): Confidence scores with shape (N,).
            iou_threshold (float): IoU threshold for suppression.
            sigma (float): Sigma for Gaussian method.
            method (str): 'linear' or 'gaussian' decay method.

        Returns:
            (torch.Tensor): Indices of boxes to keep after Soft NMS (all boxes with decayed scores > 0.001).
        """
        if boxes.numel() == 0:
            return torch.empty((0,), dtype=torch.int64, device=boxes.device)
        ...
        for i in range(len(scores)):
            ...
            if method == 'linear':
                decay = torch.clamp(ious, 0, iou_threshold) / iou_threshold
                scores[i+1:] *= (1 - decay)
            elif method == 'gaussian':
                scores[i+1:] *= torch.exp(-ious**2 / sigma)
            # Keep only boxes with score > threshold
            scores[i+1:] = torch.where(scores[i+1:] < 0.001, 0, scores[i+1:])

        # Return sorted indices of non-zero scores
        keep = torch.nonzero(scores > 0).squeeze(-1)
        return order[keep]
