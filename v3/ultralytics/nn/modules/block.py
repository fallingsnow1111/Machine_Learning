...
class BottleneckPC(nn.Module):
    """Standard Bottleneck with PConv."""
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        # 第一层卷积：原本是 Conv，现在也可以换，但通常 PConv 放在第二层或两者都换
        # 这里我们参考常见的 PConv 修改：第一层用 1x1 降维/特征变换，第二层用 PConv 提取特征
        self.cv1 = Conv(c1, c_, k[0], 1) 
        # 核心修改：将原来的 3x3 Conv 换成 PConv + 1x1 Conv (PWConv)
        # 注意：PConv 本身不改变通道数，所以通常 PConv 后面要接一个 1x1 来升维或融合
        self.cv2 = nn.Sequential(
            PConv(c_, n_div=4, forward="split_cat"),
            Conv(c_, c2, 1, 1) # Pointwise Conv to match output channels
        )
        self.add = shortcut and c1 == c2
